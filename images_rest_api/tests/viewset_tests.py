...
from .factories import CustomUserFactory, UserImageFactory
...
class TestUserImagesViewSet:
    ...
    def test_send_not_an_image(self, user):
        ...
        # Tworzymy plik, który nie jest obrazem (np. plik tekstowy)
        ...
        response = client.post(url, data, format="multipart")
        assert "Upload a valid image. The file you uploaded was either not an image or a corrupted image." in response.data["image"]
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_send_image_in_wrong_format(self, user):
        ...
        wrong_format_image = UserImageFactory.create_image("wrong_format.gif", 300, content_type="image/gif", format="GIF")
        ...
        response = client.post(url, data, format="multipart")
        assert "gif - Invalid file extension. Only ['jpg', 'jpeg', 'png'] files are accepted." in response.data["image"]
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_send_image_in_wrong_format_but_valid_name(self, user):
        ...
        wrong_format_image = UserImageFactory.create_image("wrong_format.jpg", 300, content_type="image/gif", format="GIF")
        ...
        response = client.post(url, data, format="multipart")
        assert "Cannot open the file as an image. Please check if the uploaded file is a valid image." in response.data["image"]
        assert response.status_code == status.HTTP_400_BAD_REQUEST
